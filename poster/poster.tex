...
\begin{minipage}{\linewidth}
\centering
\begin{minipage}[t]{0.48\linewidth} % Solve times
  \includegraphics[width=0.97\linewidth]{midpoint_solve_times.png}
  {\captionsetup{hypcap=false}%
  \captionof{figure}{Median solve times (ms) across Rössler variants using a fixed-step \texttt{Midpoint} solver.  Lower bars indicate faster solves; baseline is the naïve out-of-place RHS.}}
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\linewidth} % RHS + allocations
  \includegraphics[width=0.97\linewidth]{rhs_allocations.png}
  {\captionsetup{hypcap=false}%
  \captionof{figure}{Median RHS time and per-call allocations for each variant.  Eliminating heap allocations delivers the largest performance gains.}}
\end{minipage}
\end{minipage}

\medskip
\begin{center}
  \input{figures/midpoint_table.tex}
\end{center}
...
\begin{center}
  \begin{minipage}[t]{0.7\linewidth}
    \centering
    \includegraphics[width=0.98\linewidth]{trajectory.png}
    {\captionsetup{hypcap=false}%
    \captionof{figure}{Phase-space trajectory for the optimized Rössler implementation, illustrating qualitative agreement across variants.}}
  \end{minipage}
\end{center}
